            ...
            if let Some((start_pos, start_len)) = find_start_code(buf) {
                ...
                if let Some((next_pos, _next_len)) = find_start_code(search_buf) {
                    ...
                    if !nal_data.is_empty() {
                        ...
                        nals.push(NalUnit { nal_type, data: nal_data });
                    }
                    continue;
                } else {
                    break;
                ...
                if self.buffer.len() > 1024 * 1024 { self.buffer.clear(); }
                break;
            }
        }
        nals
...
fn find_start_code(buf: &[u8]) -> Option<(usize, usize)> {
    ...
    for i in 0..buf.len() - 2 {
        if buf[i] == 0 && buf[i+1] == 0 {
            if buf[i+2] == 1 { return Some((i, 3)); }
            if i + 3 < buf.len() && buf[i+2] == 0 && buf[i+3] == 1 { return Some((i, 4)); }
        }
    }
    None
}
...
impl H264Packetizer {
    pub fn new(mtu: usize, payload_type: u8, clock_rate: u32) -> Self {
        Self {
            ...
            clock_rate,
            cached_sps: None, cached_pps: None,
            sps_pps_timestamp: 0,
        }
    }
    ...
    pub fn packetize(&mut self, nal: &NalUnit) -> Vec<RtpPacket> {
        ...
        match nal.nal_type {
            NalType::Sps => { self.cached_sps = Some(nal.data.clone()); self.sps_pps_timestamp = self.timestamp; }
            NalType::Pps => { self.cached_pps = Some(nal.data.clone()); self.sps_pps_timestamp = self.timestamp; }
            NalType::Idr => {
                if self.sps_pps_timestamp != self.timestamp {
                    if let Some(sps) = &self.cached_sps { packets.push(self.create_single(sps.clone(), false)); }
                    if let Some(pps) = &self.cached_pps { packets.push(self.create_single(pps.clone(), false)); }
    ...
    fn create_single(&mut self, data: Bytes, marker: bool) -> RtpPacket {
        let p = RtpPacket {
            header: RtpHeader {
                ...
                payload_type: self.payload_type, sequence: self.sequence,
                timestamp: self.timestamp, ssrc: self.ssrc,
            },
            payload: data,
        };
    ...
    fn create_fua(&mut self, nal: &NalUnit) -> Vec<RtpPacket> {
        ...
        while offset < payload.len() {
            ...
            f_payload.extend_from_slice(&[nri | 28, ((start as u8) << 7) | ((end as u8) << 6) | typ]);
            f_payload.extend_from_slice(&payload.slice(offset..offset+size));

            packets.push(RtpPacket {
                header: RtpHeader {
                    version: 2, padding: false, extension: false, marker: end && nal.nal_type.is_vcl(),
                    payload_type: self.payload_type, sequence: self.sequence,
                    timestamp: self.timestamp, ssrc: self.ssrc,
                },
    ...
    pub fn cached_pps(&self) -> Option<Bytes> { self.cached_pps.clone() }
...
pub fn parse_profile_level_id(sps: &[u8]) -> Option<String> {
    if sps.len() >= 4 { Some(format!("{:02x}{:02x}{:02x}", sps[1], sps[2], sps[3])) } else { None }
}
