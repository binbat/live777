...
type NALCallbackFFI = unsafe extern "C" fn(
    data: *const u8,
    size: usize,
    is_keyframe: c_int,
    timestamp: u64,
...
unsafe extern "C" {
    ...
    fn bridge_set_callback(
        ctx: *mut BridgeContext,
        callback: NALCallbackFFI,
        user_data: *mut c_void,
    );
...
impl LibcameraSource {
    ...
    async fn build_video_codec_params(&self) -> RTCRtpCodecParameters {
        ...
        let profile = self
            ...
            .await
            .clone()
            .unwrap_or_else(|| self.params.profile.clone());
...
impl StreamSource for LibcameraSource {
    ...
    async fn start(&mut self) -> Result<()> {
        ...
        info!(
            "[{}] Starting libcamera-bridge (V14.8-SYNC)",
            self.config.stream_id
        );

        let ctx_raw = unsafe {
            bridge_init(
                self.params.width as c_int,
                self.params.height as c_int,
                self.params.fps as c_int,
                self.params.bitrate as c_int,
                self.params.camera_id as c_int,
                self.params.rotation as c_int,
                if self.params.hflip { 1 } else { 0 },
                if self.params.vflip { 1 } else { 0 },
            )
        };
        if ctx_raw.is_null() {
            anyhow::bail!("Failed to initialize libcamera bridge");
        }
        ...
        unsafe {
            bridge_set_callback(ctx.0, Self::nal_callback, user_data_raw);
        }
    ...
    async fn stop(&mut self) -> Result<()> {
        ...
        if let Some(ctx) = self.bridge_ctx.take() {
            unsafe {
                bridge_stop(ctx.0);
                bridge_free(ctx.0);
            }
        }
        if let Some(ptr) = self._user_data_holder.take() {
            unsafe {
                let _ = Box::from_raw(ptr as *mut mpsc::UnboundedSender<NALMessage>);
            }
        }
