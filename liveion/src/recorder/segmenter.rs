...
impl Segmenter {
    ...
    /// `duration_ticks` â€?duration in the 48 kHz time base (i.e. RTP timestamp delta)
...
<MPD xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"\n\
    ...
    async fn store_file(&self, name: &str, data: Vec<u8>) -> Result<()> {
        ...
        // Spawn the actual write in a detached task so that slow/objectâ€storage latency does
        // not block the realâ€time RTP processing loop. Any error will be logged.
