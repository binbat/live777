...
impl PliBackoff {
    ...
    /// Create a conservative backoff strategy (longer timeouts, fewer retries)
    ...
    /// Create an aggressive backoff strategy (shorter timeouts, more retries)
    ...
    /// Create a linear backoff strategy (no exponential growth)
    pub fn linear(timeout: Duration, max_requests: u32) -> Self {
        Self::new(
            timeout,
            timeout,
            max_requests,
    ...
    /// Get number of consecutive requests without keyframe
    pub fn request_count(&self) -> u32 {
        self.request_count
    }
    
    /// Get total number of PLI requests sent (lifetime statistics)
    pub fn total_requests(&self) -> u64 {
        self.total_requests
    }
    
    /// Get total number of keyframes received (lifetime statistics)
    ...
    /// Check if backoff has reached maximum (giving up on requests)
    ...
    pub fn state_summary(&self) -> String {
        format!(
            ...
            self.request_count,
            if self.max_requests == 0 { "∞".to_string() } else { self.max_requests.to_string() },
...
mod tests {
    ...
    fn test_max_requests() {
        ...
        assert!(!backoff.should_request(), "Should not request after max attempts");
        assert!(backoff.is_maxed_out());
    }
    
    #[test]
    fn test_linear_backoff() {
        let mut backoff = PliBackoff::linear(Duration::from_secs(2), 5);
