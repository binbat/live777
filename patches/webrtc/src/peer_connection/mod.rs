...
mod tests {
    use super::*;
    use crate::runtime::{Receiver, block_on, channel, timeout};
    ...
    fn new_test_peer_connection() -> (PeerConnectionImpl, Receiver<PeerConnectionDriverEvent>) {
    ...
    #[test]
    fn add_udp_ice_candidate_wakes_driver() {
        block_on(async {
            let (pc, mut driver_event_rx) = new_test_peer_connection();
            let remote_offer = {
                let mut remote = RTCPeerConnectionBuilder::new().build().unwrap();
                let _ = remote.create_data_channel("test", None).unwrap();
                remote.create_offer(None).unwrap()
            };
            pc.set_remote_description(remote_offer).await.unwrap();
            let _ = driver_event_rx.recv().await;

            let candidate = RTCIceCandidateInit {
                candidate: "candidate:0 1 udp 2130706431 127.0.0.1 3478 typ host".to_string(),
