...
- publishes a VP8 test stream via WHIP using a built-in `rsmpeg`-based source
...
The test source uses FFmpeg libraries through `rsmpeg`, so the system must
provide the FFmpeg development libraries (probed via `pkg-config`).
