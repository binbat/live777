...
pub fn route() -> Router<AppState> {
    Router::new()
        ...
        .route("/manager/streams/:stream/:addr/detail", get(stream_node_detail))
}

async fn nodes(
    State(state): State<AppState>,
    Query(qry): Query<NodeQuery>,
) -> Result<Json<Page<Node>>> {
    let count = Node::db_query_count(&qry, &state.pool).await?;
    let mut page = Page::new(qry.page, qry.page_size, count);
...
async fn node_detail(
    ...
    let addr = Node::db_find_by_addr(&state.pool, addr).await?.ok_or_else(|| AppError::ResourceNotFound)?;
...
impl Node {
    ...
    fn db_query_where(query_build: &mut QueryBuilder<MySql>, node_query: &NodeQuery, limit: bool) {
        ...
        if limit {
            query_build
                .push(" limit ")
                .push_bind((node_query.page - 1) * node_query.page_size)
                .push(" , ")
...
impl Stream {
    ...
    fn db_query_where(query_build: &mut QueryBuilder<MySql>, node_query: &StreamQuery, limit: bool) {
        if let Some(addr) = &node_query.addr {
            query_build.push(" and addr = ").push_bind(addr.clone());
        }
