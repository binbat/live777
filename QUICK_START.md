# 快速开始指南

## 🎉 系统已完全正常工作！

Liveion UDP 桥接系统现在可以完美运行，实现网页端到 UDP 设备的双向通信。

## 🚀 快速启动

### 一键启动所有服务

```cmd
start_complete_demo.bat
```

这个脚本会自动启动：
- ✅ Liveion 服务器 (端口 7777)
- ✅ UDP 桥接程序 (端口 8888) 
- ✅ UDP 测试监听器 (端口 8889)
- ✅ Python HTTP 服务器 (端口 8080)
- ✅ Web 控制界面

## 📝 使用步骤

1. **启动所有服务** - 运行 `start_complete_demo.bat`
2. **等待启动完成** - 约 10 秒
3. **打开 Web 界面** - 浏览器自动打开 `http://localhost:8080/examples/liveion_udp_control.html`
4. **点击连接** - 点击"连接"按钮
5. **测试控制** - 使用方向键或点击按钮发送云台控制指令
6. **查看日志** - 在 UDP 监听器窗口查看接收到的消息

## 🎯 预期结果

### 正常运行的标志

1. **Liveion 服务器窗口**: 
   ```
   Server listening on http://0.0.0.0:7777
   ```

2. **UDP 桥接程序窗口**:
   ```
   🚀 Starting liveion UDP bridge
   📋 Loaded configuration from "bridge.toml"
   🌉 Bridge created successfully
   ✅ WebRTC connection established
   📡 DataChannel 已打开 [ID: 0]
   ```

3. **Web 界面**:
   ```
   ✅ 连接成功！
   ✅ DataChannel 已打开
   DataChannel + UDP桥接已连接
   ```

4. **UDP 监听器窗口**:
   ```
   UDP 监听器启动，端口: 8889
   [UDP收到] 来自 ('127.0.0.1', 8888): {"action":"pan","direction":"left",...}
   ```

## 🔧 手动启动（可选）

如果需要单独启动各个组件：

```cmd
# 1. 启动 liveion 服务器
target\release\live777.exe --config conf\live777.toml

# 2. 启动 UDP 桥接程序
target\release\liveion-udp-bridge.exe -v

# 3. 启动 UDP 监听器
python test_liveion_udp.py listen

# 4. 启动 HTTP 服务器
python -m http.server 8080

# 5. 打开浏览器
# 访问 http://localhost:8080/examples/liveion_udp_control.html
```

## 🛑 停止服务

关闭所有命令行窗口，或运行：
```powershell
./stop_complete_demo.ps1
```

## 🔍 故障排除

### 端口冲突
如果遇到端口占用错误：
```cmd
# 检查端口占用
netstat -ano | findstr "7777 8888 8889 8080"

# 终止占用进程
taskkill /f /im live777.exe
taskkill /f /im liveion-udp-bridge.exe
taskkill /f /im python.exe
```

### 连接失败
1. 确保按顺序启动服务
2. 等待 liveion 服务器完全启动（约 5 秒）
3. 使用 HTTP 服务器访问页面，不要直接打开 HTML 文件

### UDP 监听器无消息
1. 检查桥接程序是否显示 "DataChannel 已打开"
2. 确认网页显示"已连接"状态
3. 查看桥接程序日志是否有错误

## 📞 测试命令

```cmd
# 发送测试 UDP 命令
python test_liveion_udp.py test

# 查看帮助
target\release\liveion-udp-bridge.exe --help
```

## 🎮 控制说明

### 键盘控制
- **方向键**: 云台上下左右
- **空格键**: 停止
- **+/-**: 变焦

### 鼠标控制
点击界面上的控制按钮

## 🔑 关键配置

### bridge.toml
```toml
[udp]
listen = "0.0.0.0"
port = 8888
target_addresses = ["127.0.0.1:8889"]

[liveion]
url = "http://localhost:7777"
stream = "webcontrol"

[bridge]
enable_logging = true
```

### conf/live777.toml
```toml
[http]
port = 7777
cors = true

[whep]
auto_create_whep = true
```

## ✅ 系统架构

```
网页端 (WHIP发布者)
    ↓ DataChannel
Liveion 服务器
    ↓ DataChannel  
UDP 桥接程序
    ↓ UDP (端口 8888)
UDP 设备 (端口 8889)
```

现在你可以开始使用完整的 UDP 控制系统了！🎊
