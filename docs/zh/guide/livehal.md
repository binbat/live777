...
`GET /api/sources/:streamId/bitrate` 报告该源编码器码率当前的驱动
方式 —— `adaptive`(上面的 AIMD 控制器)或 `fixed` —— 以及当前
码率(见 [HTTP API 文档](./live777-api.md))。控制通过**质量档位**
进行:命名的源参数预设,通过 `POST /api/sources/:streamId/tier`
应用。档位是源自身结构的部分覆盖 —— 可选的 `capture` 和
`encoder` 块里放要改的参数;未写的参数保持配置值:
...
# 阶梯档:采集几何 + 编码预算,通过重建采集+编码管线切换
# (亚秒级断帧,订阅者保持连接)。这里省略 encoder.bitrate ——
# 按 640x480x15 以 ~0.07 bpp 推导(约 645 kbps)
[[stream.pi-cam.sources.tiers]]
name = "low"
...
档位横跨两个块:`encoder` 覆盖调编码器,而 `capture` 覆盖要改采集
配置(livehal 没有 scaler 环节 —— 采集尺寸就是编码器输入尺寸,
帧率也是采集侧属性)。纯编码器档原地无缝切换;动了采集几何的档位
会在流的下方重建管线 —— RTP 会话保持存活,订阅者只会看到
短暂定格和一次带内 SPS/PPS 变更。`capture.width`/`height` 必须成对
设置,可以是传感器支持的任意尺寸 —— 包括高于启动配置(档位阶梯的
顶端不必是启动档,例如 972p30 启动配 1080p20 最高档);`capture.fps`
同样可以是传感器支持的任意帧率(低延时档可以用分辨率换帧率,例如
OV5647 上的 480p60 档 —— 不支持的模式会在应用时被拒绝并回滚)。
`encoder.bitrate` 同样可以高于启动的 `encoder.bitrate`(仅受编码器
有符号 32 位控制范围约束),因此阶梯顶端可以携带比启动档更高的码率
—— 而且它是*可选*的:省略时按档位的生效几何以 ~0.07 bpp 推导
(下限 50 kbps),手工调优的阶梯通常也落在这个附近;内容特殊(如
运动场景)或想在同分辨率出高清/经济两档时再显式指定。空档位
(什么都没设置)会被拒绝。

应用档位*不会*挂起自适应控制器:档位的码率成为 AIMD 的新上限,
它会在档内继续跟随网络状况调节。重建失败(例如摄像头拒绝新尺寸)
会回滚到之前的配置;档位的字段总是叠加在配置的采集参数上:未
设置的字段回落到配置值,因此混合阶梯(纯码率档与几何档并存)语义
是明确的。同一机制日后还可以扩展到切换源的*类型*(例如从摄像头
切到 RTSP 地址)——档位描述的是源配置,而不是编码器内部。
