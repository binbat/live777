...
短暂定格和一次带内 SPS/PPS 变更。`width`/`height` 必须成对设置,
可以是传感器支持的任意尺寸 —— 包括高于启动配置(档位阶梯的顶端
不必是启动档,例如 972p30 启动配 1080p20 最高档);`fps` 同样可以是
传感器支持的任意帧率(低延时档可以用分辨率换帧率,例如 OV5647 上
的 480p60 档 —— 不支持的模式会在应用时被拒绝并回滚)。`bitrate`
同样可以高于启动的 `encoder.bitrate`(仅受编码器有符号 32 位控制
范围约束),因此阶梯顶端可以携带比启动档更高的码率 —— 而且它对
几何档位是*可选*的:省略时按生效的 `width`×`height`×`fps` 以
~0.07 bpp 推导(下限 50 kbps),手工调优的阶梯通常也落在这个
附近;内容特殊(如运动场景)或想在同分辨率出高清/经济两档时再
显式指定。纯码率档(无几何参数)仍必须显式给出 `bitrate`。
