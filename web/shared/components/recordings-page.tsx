...
export function RecordingsPage() {
    ...
    const [indexEntries, setIndexEntries] = useState<api.RecordingIndexEntry[]>([]);
    ...
    const fetchIndex = useCallback(async () => {
        ...
        try {
            setLoading(true);
            setError('');
            const res = await api.getRecordingIndexByStream(selectedStream);
                            ...
                            <div key={ e.record } className="border border-base-200 rounded-lg p-3 flex flex-col gap-2">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <Tooltip message="Copy MPD URL">
                                        <Button size="sm" color="ghost" onClick={() => copyToClipboard(new URL(api.getSegmentUrl(e.mpd_path), location.origin).toString())}>
